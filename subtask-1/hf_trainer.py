...
if args.augmentation is not None:
    dir_name = f"{name}-{args.model_type_arch}-{args.context}-{args.augmentation}-{args.percentage_tags}-{args.random_seed}-roberta-es-ner-trainer-v5"
else:
    dir_name = f"{name}-{args.model_type_arch}-{args.context}-{args.random_seed}-roberta-es-ner-trainer-v5"
...
train_ds, test_ds = load_train_test_split("dataset/medprocner_train",
                                          tokenizer=tokenizer,
                                          context_size=CONTEXT_SIZE,
